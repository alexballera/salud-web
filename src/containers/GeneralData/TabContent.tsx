...
const TabContent = ({tab}: IProps): JSX.Element => {
  ...
  const renderDoctor = () => {
    ...
    return <Typography className={classes.smallText}>
    {t('content.lastMeasurement', { ns: i18nGeneralData })}
    <br />
    <br />
    {t('label.for', { ns: i18nGlobalsData })} {dr.length > 0 && dr[0]}
    <br />
  ...
  return (
    ...
    <Grid container className={classes.paddingContainer} justify='space-around'>
      <Grid item xs={5}>
        {tab === 0 && <MeasurementCard
          title={t('tabs.bloodGlucose', { ns: i18nGeneralData })}
          noSVG
          value={data.bloodGlocuse.generalData.value > 0 ? data.bloodGlocuse.generalData.value.toString() : '-'}
        />}
        {tab === 1 && <MeasurementCard
          title={t('tabs.pressure', { ns: i18nGeneralData })}
          noSVG
          value={`${data.arterialPressure.generalData.systolic > 0 ? data.arterialPressure.generalData.systolic.toString(): '-'}/${data.arterialPressure.generalData.diastolic > 0 ? data.arterialPressure.generalData.diastolic.toString() : '-'}`}
          />}
        {tab === 2 && <MeasurementCard
